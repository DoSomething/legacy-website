...
class ReactionsList extends React.Component {
  ...
  toggleReaction(termId) {
    ...
    if (reactionId) {
      sendEvent('Onboarding', 'Un-like'); // Soon to be removed.
      sendEvent('onboarding-v2', 'unlike', this.constructor.name);
    ...
    if (!reactionId) {
      sendEvent('Onboarding', 'Like'); // Soon to be removed.
      sendEvent('onboarding-v2', 'like', this.constructor.name);
