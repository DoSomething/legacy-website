...
const bodies = ["By signing up for the [CAMPAIGN NAME] campaign, you’ve teamed up with 5.3 million other DoSomething.org members who are making positive change right now.", "These members (and thousands of others!) are already taking action on [CAMPAIGN NAME], so tap the heart to show them some love. Then get started! Do the campaign, take photos of yourself in action, and upload them to the site. LET’S DO THIS."];
...
class ReportbackItemsSlide extends React.Component {
  constructor(props) {
    ...
    if (competition && competition.joined && competition.messages) {
      bodies.push(competition.messages.confirmation);
    }
  }
          ...
          <div className="wrapper">
            ...
            <div className="container__row">
              <div className="container__block">
                ...
                {bodies.map(body => {
                  return <p>{body.replace('[CAMPAIGN NAME]', this.props.campaign.title)}</p>
                })}
              </div>
            </div>
