define(["jquery", "lodash"], function($, _) {
  ...
  var SolrAdapter = {
     ...
     **/
    baseURL: "http://98.129.111.172/solr/",
    ...
    defaultQuery: [
      ...
      "rows=10",
      "fl=label,tid,im_vid_1,sm_vid_Action_Type,tm_vid_1_names,im_field_cause,im_vid_2,sm_vid_Cause,tm_vid_2_names,im_field_tags,im_vid_5,sm_vid_Tags,tm_vid_5_names,fs_field_active_hours,sm_field_call_to_action,bs_field_staff_pick,ss_field_search_image"
    ],
    ...
    query: function (checkedFields, successCallback) {
      ...
      SolrAdapter.xhr = $.ajax({
        ...
        url: 'http://dosomething.org', //SolrAdapter.buildQuery(query, q),
        success: function (data) {
          successCallback(data);
        },
        error: function(equest, errorType, errorMessage) {
          var $message = $(document.createElement('div')).addClass('finder--message fade-in'),
            $finderForm = $('.finder--form');

          $message.text("Ooof! We're not sure what's up? Maybe it's us, or it could be your internet connection. Want to try again in a few minutes?");
          $finderForm.find('.home--hero').append($message);
        },
