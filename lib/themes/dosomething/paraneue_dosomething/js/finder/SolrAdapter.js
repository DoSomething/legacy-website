define(["jquery", "lodash", "finder/CampaignFinder"], function($, _, CampaignFinder) {
  "use strict";
  ...
  var SolrAdapter = {
     ...
     * @param successCallback   Function called when query successfully completes.
     */
    query: function ($fields, successCallback) {
      /**
       * A clone of the default query so we don't modify it directly
       * @type {Array}
       */
      var query = SolrAdapter.defaultQuery.slice(0);
      ...
      for (var i in $fields) {
        var checked = $fields[i].filter(":checked");
        ...
        if (checked.length > 0) {
          ...
          /*
             ...
             */
          var powerset = SolrAdapter.generatePowerset(queryBuilder);
          if (powerset.length) {
            param += ' OR ' + powerset;
          }

          // Push the newly built parameter onto the q parameter
          q.push(SolrAdapter.fieldMap[i] + ":" + '(' + encodeURIComponent(param) + ')');
        }
      }
