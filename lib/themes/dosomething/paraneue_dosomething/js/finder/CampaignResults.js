define(["jquery", "unveil", "finder/Campaign"], function($, unveil, Campaign) {
  ...
  var CampaignResults = {
    ...
    init: function ($div, $blankSlateDiv) {
      ...
      CampaignResults.$div.hide();

      CampaignResults.$blankSlateDiv = $blankSlateDiv;

      // Lazy-load in images
      CampaignResults.$blankSlateDiv.find("img").unveil(200, function() {
        $(this).load(function() {
    ...
    clear: function () {
      ...
      CampaignResults.$div.show();
      CampaignResults.$blankSlateDiv.hide();
