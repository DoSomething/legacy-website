...
import socialDeclarationTpl from 'campaign/templates/OrganDonation/declaration.tpl.html';
...
class OrganDonation extends Takeover {
  constructor($takeoverContainer, tpl) {
    ...
    this.$declarationScreen = this.$el.find('.js-social-declaration');
  ...
  loadZipCode() {
    ...
    this.$zipCodeButton.click(e => {
      ...
      const $birthdayField = this.$el.find('input[name="birthday"]');
      const birthday = $birthdayField.val();
      const $zipCodeField = this.$el.find('input[name="postal_code"]');
      const zipCode = $zipCodeField.val();

      var $split_birthday = birthday.split('/');
      var $month = parseInt($split_birthday[0], 10) - 1;
      var $day = parseInt($split_birthday[1], 10);
      var $year = parseInt($split_birthday[2], 10);
      var $compareDate = new Date($year + 18, $month, $day);
  ...
  /*
   * Handles the social declaration screen.
   */
  loadDeclaration() {
    this.$registrationButton.off('click');

    this.loadScreen(this.$declarationScreen);
    this.addTemplate(this.$declarationScreen, socialDeclarationTpl, null);
  ...
  set() {
    ...
    if (this.isActive(this.$declarationScreen)) {
      this.loadDeclaration();
    }
