    ...
    Validation.Events.subscribe('Validation:Submitted', (topic, args) => {
      ...
      .then(json => {
        if (typeof json.uuid !== 'undefined') {
          ...
          const $errorContainer = this.$el.find('.js-organize-errors');
          $errorContainer.html('');

          $errorContainer.append('<div class="validation__message has-error"><strong>There were some errors with your form:</strong></div>');
          ...
          for (let error in json) {
            if (json.hasOwnProperty(error)) {
              if (error === 'agree_to_tos') {
                $errorContainer.append('<li class="validation__message has-error">You must agree to the terms of service</li>');
              } else {
                $errorContainer.append('<li class="validation__message has-error">' + json[error] + '</li>');
              }
            }
          }
