...
class OrganDonation extends Takeover {
  ...
  loadRegistration(formConfig) {
    ...
    const $registrationButton = $('.submit-registration');
    ...
    $registrationButton.click(e => {
      ...
      const values = $('.organize-registration-form').serialize();
      ...
      // If errors, display validation message to fix issues.
    });
   ...
   */
  loadShare() {
    this.loadScreen($('.js-share-confirmation'));
    this.resizeContainerHeight(this.$el.find('.takeover'), $('.js-share-confirmation'));
  }
