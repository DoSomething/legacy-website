...
class OrganDonation extends Takeover {
  constructor($takeoverContainer, tpl) {
    ...
    this.email               = setting('dosomethingUser.email');
    this.first_name          = setting('dosomethingUser.first_name');
    this.last_name           = setting('dosomethingUser.last_name');
    this.birthdate           = setting('dosomethingUser.birthday');
    this.street_address      = setting('dosomethingUser.street_address');
    this.street_address_2    = setting('dosomethingUser.street_address_2');
    this.city                = setting('dosomethingUser.city');
    this.state               = setting('dosomethingUser.state');
      ...
      fieldsets.fields.forEach(field => {
        ...
        for (const entry of fieldToValidationMap.entries()) {
          if (field.field_name === entry[0]) {
            ...
            if (field.field_name in prefillValues) {
              field.pre_fill_value = prefillValues[field.field_name];
            }
          }
        }
