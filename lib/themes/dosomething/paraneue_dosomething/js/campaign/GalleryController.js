...
class GalleryController {
  ...
  getCampaigns(args) {
    ...
    return new Promise((resolve, reject) => {
      this.apiClient.get('campaigns', query).then(response => {
        let campaigns = response.data;

        // Remove any campaigns we've already fetched for this gallery.
        if (args['removeDuplicateResults']) {
          campaigns = campaigns.filter(campaign => this.retrievedCampaigns.indexOf(parseInt(campaign.id)) == -1);
        }
