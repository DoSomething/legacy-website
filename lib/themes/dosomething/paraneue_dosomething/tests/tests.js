...
require(["neue/validation", "validation/auth"], function(Validation, Auth) {
  ...
  test("Reportback numbers", function() {
    Validation.Validations.reportbackNumber.fn("19", function(result) {
      ok(result.success == true, "should accept a valid integer");
    });

    Validation.Validations.reportbackNumber.fn(" 5", function(result) {
    ...
    Validation.Validations.reportbackNumber.fn("two", function(result) {
      ok(result.success == false, "should reject a number spelled out as a string");
    });

    Validation.Validations.reportbackNumber.fn("4.5", function(result) {
      ok(result.success == false, "should reject a decimal number");
    });

    Validation.Validations.reportbackNumber.fn("0", function(result) {
